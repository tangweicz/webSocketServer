...
class webSocketServer(object):
            ...
            for sock, event in allSocketHandle:
                if event == select.EPOLLIN:  # 有可读事件
                    if sock == self.sock.fileno():
                        ...
                        self.epollHandle.register(client.fileno(), select.EPOLLIN)
                    else:
                        ...
                        if message:
                            if not self.dictSocketShakeHandStatus[sock]:
                                ...
                                if headerData:
                                    ...
                                    self.epollHandle.modify(sock, select.EPOLLOUT)
                            else:
                                if message["type"] == 1:
                                    print("数据已传输完毕，纯文本消息")
                                    ...
                                    self.dictSocketRecvData.pop(sock)
                                    print("关闭操作完成")
                                elif message["type"] == 2:
                    ...
                    self.epollHandle.modify(sock, select.EPOLLIN)
    ...
    def sendMessage(self, sockHandle, message, filename = ""):#用户提交数据的数据
        # print("准备发送数据到客户端.....\n")
        ...
        if not self.dictSocketShakeHandStatus[sockHandle]:
            ...
            if not sockHandle in self.dictSocketSendHandle:
                if filename == "":
        ...
        if not sockHandle in self.dictSocketSendHandle:
            self.dictSocketSendHandle[sockHandle] = len(strings)  # 要发送数据的总长度
        if not sockHandle in self.dictSocketSendedHandle:
            self.dictSocketSendedHandle[sockHandle] = 0  # 已发送数据总长度
        if not self.dictSocketShakeHandStatus[sockHandle]:
            self.dictSocketShakeHandStatus[sockHandle] = True
        # print("要发送数据总长度：", totalLen)
        print("yyyyy")

        while self.dictSocketSendedHandle[sockHandle] < self.dictSocketSendHandle[sockHandle]:
            try:
            ...
            if "firstByteFIN" in self.dictSocketRecvData[sockHandle]:
                ...
                if "payload_len" in self.dictSocketRecvData[sockHandle]:
                    ...
                    if self.dictSocketRecvData[sockHandle]["payload_len"] == 126:
                        print("========", self.dictSocketRecvData[sockHandle])
                        ...
                        if not "dataLength" in self.dictSocketRecvData[sockHandle]:
                            ...
                            if self.dictSocketRecvData[sockHandle]["recvExtendMinPayloadLen"] < minExtendPayloadLen:
                                try:
                                    ...
                                    if len(extend_payload_len) == 1:
                                        ...
                                        # self.dictSocketRecvData[sockHandle]["dataLength"] = struct.unpack("!H", extend_payload_len)[0]  # 因为是2个字节 所以用H解码
                                except RuntimeError as re:
                                    pass
                            else:
                        ...
                        if "dataLength" in self.dictSocketRecvData[sockHandle]:
                            if not "mask" in self.dictSocketRecvData[sockHandle]:
                                ...
                                if self.dictSocketRecvData[sockHandle]["recvMaskLen"] < maskRecvLength:
                                    try:
                        ...
                        if not "dataLength" in self.dictSocketRecvData[sockHandle]:
                            ...
                            if self.dictSocketRecvData[sockHandle]["recvExtendMaxPayloadLen"] < maxExtendPayloadLen:
                                try:
                        ...
                        if "dataLength" in self.dictSocketRecvData[sockHandle]:
                            ...
                            if not "mask" in self.dictSocketRecvData[sockHandle]:
                                ...
                                #         raise RuntimeError()
                                #     if len(mask) >= 4:
                                #         self.dictSocketRecvData[sockHandle]["mask"] = mask
                                # except RuntimeError as re:
